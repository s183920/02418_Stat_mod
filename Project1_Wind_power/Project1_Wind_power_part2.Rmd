...
summary(model2.2)
model2.3 <- glm(pow.obs.norm ~ ws30 + I(ws30^2) + I(pts_on_unit_circle^3), family=Gamma, data=wind_data)
...
```{r}
#Gamma
model3 <- glm(pow.obs.norm ~ ws30 + I(ws30^2) + I(pts_on_unit_circle^3), family=Gamma(log),  data=wind_data)
summary(model3)
AIC(model3) #-333.9629
...
plot(wind_data$ws30, wind_data$pow.obs.norm, pch = 16, xlab = , ylab = "VS")
...
empirical.mod<-locfit(pow.obs.norm ~ ws30 + pts_on_unit_circle, data=wind_data, family="gamma")
