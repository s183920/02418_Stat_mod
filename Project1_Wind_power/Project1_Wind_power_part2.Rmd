...
```{r}
#series expnsion
model2 <- glm(pow.obs.norm ~ ws30 + I(ws30^2) + I(wd30)^3, data=wind_data)
summary(model2)
AIC(model2)


#Gamma
model3 <- glm(pow.obs.norm ~ ws30 + I(ws30^2), family=Gamma,  data=wind_data)
summary(model3)
AIC(model3)

#Beta
library(betareg)
model4 <- betareg(pow.obs.norm ~ ws30, data=wind_data)
summary(model4)
AIC(model4) #-462.8008
...
summary(model6)
AIC(model6) #hvorfor bliver denne så stor???
```
```{r}
