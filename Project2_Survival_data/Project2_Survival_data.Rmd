...
nll <- function(theta, data){
  l1 <- act_data %>% filter(event == 1) %>% pull(time) %>% dexp(., rate = theta) %>% log() %>% sum()
  l2 <- act_data %>% filter(event == 0) %>% pull(time) %>% 1-pexp(., rate = theta) %>% log() %>% sum()
 # -sum(log(dexp(data[data["event"] == 1]["time"], rate = theta))) - sum(log(pexp(data[data["event"] == 0]["time"], rate = theta)))
 -l1-l2
}
act_data %>% filter(event == 1) %>% pull(time) %>% dexp(rate = theta) %>% log() %>% sum()
act_data %>% filter(event == 0) %>% pull(time) %>% pexp(rate = theta) %>% log()
