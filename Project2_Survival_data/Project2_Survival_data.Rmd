...
cat("Summary table:\n")
summary_table %>% xtable::xtable(caption = "The proportions of people who died or got AIDS in the two treatment groups", label = "survival_props")
...
plot(theta, sapply(theta, nll, data = act_data) - opt$objective, 'l', ylab = "Log likelihood", main = "All data")
...
plot(theta, nll(theta, act_data %>%  filter(tx == 0)) - opt0$objective, 'l', main = "Test for difference in rate in the 2 groups", ylab = "Log likelihood", ylim = c(-10,0))
...
CI_tx0_array <- theta[nll(theta, act_data %>% filter(tx == 0)) - opt0$objective > c]
CI_tx0 <- c(min(CI_tx0_array), max(CI_tx0_array))

CI_tx1_array <- theta[nll(theta, act_data %>% filter(tx == 1)) - opt1$objective > c]
CI_tx1 <- c(min(CI_tx1_array), max(CI_tx1_array))

cat("Confidence intervals for tx = 0:", CI_tx0, "\n")
cat("confidence intervals for tx = 1:", CI_tx1)
...
nll_combined <- function(theta, data){
  ...
  -(nll(rate0, data %>% filter(tx == 1)) + nll(rate1, data %>% filter(tx == 0)))
...
b1 <- seq(0,2, 0.1)
plot(b1, -sapply(b1, nll_profile, data = act_data)+opt_b1$objective , 'l', main = "Profile likelihood for b1", ylab = "log likelihood")
abline(h= c_l)
abline(v = opt_b1$par)
...
plot(b1, sapply(b1, ll_profile, data = act_data) - opt$objective, 'l')
...
MLE_b0 = -log( (a+b)/(c+exp(-0.699172)) )
MLE_B1 = -(log(b/d2)+7.6203 )
