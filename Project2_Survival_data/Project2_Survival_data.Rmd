...
```{r}
CI_wald <- function(theta_hat, info){theta_hat+c(-1,1)*qnorm(1-0.05/2)*1/sqrt(info)}

hess <- as.numeric(numDeriv::hessian(nll_combined, opt_bs$par, data = act_data))

cat("95% wald confidence interval for b0 is: ", CI_wald(opt_bs$par[1], hess[1]))
cat("\n95% wald confidence interval for b1 is: ", CI_wald(opt_bs$par[2], hess[4]))
