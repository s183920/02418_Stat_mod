...
# png("Plots/beta1_pl_numerical.png") # start saving
plot(b1, -sapply(b1, nll_profile, data = act_data)+opt_b1$objective , 'l', main = expression(paste("Profile likelihood for ", beta[1])), ylab = "Log-likelihood", xlab = expression(beta[1]))
plot(b1, sapply(b1, quad_log), "l", col = "blue")
abline(h= c, col = "red")
abline(v = opt_b1$par)
abline(h = -nll_profile(CI_wald(opt_bs$par[2], hess[2,2]), data = act_data)[1]+opt_b1$objective, col = "green")
legend("bottomright",legend = c("numerical CI", "Wald CI"), fill = c("red", "green"))
# dev.off() #stop saving
