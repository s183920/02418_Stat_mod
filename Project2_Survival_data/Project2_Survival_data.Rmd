...
opt_rate <- optimise(nll, c(1e-10, 1e-3), data = act_data)
...
abline(v = opt_rate2$par)
abline(h = log(c))
abline(v = rate$minimum)
...
opt0 <- optimize(nll, c(1e-10, 1e-1), data = act_data %>% filter(tx == 0), maximum = T)
opt1 <- optimize(nll, c(1e-10, 1e-1), data = act_data %>% filter(tx == 1), maximum = T)

exp(opt1$objective)/exp(opt0$objective)
AIC0 <- -2*opt0$objective + 2
  AIC1 <- -2*opt1$objective + 2
```
