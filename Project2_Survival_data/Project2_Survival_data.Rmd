...
A little bit of summary statistics .... Proportion of death/AIDS in each group...
```{r}
act_data <- act_data %>% 
...
```{r}
data <- act_data$time
likelihood_gamma <- function(theta, data){
    -sum(dgamma(data, shape = theta[1], rate = theta[2], log = T))
}
likelihood_pois <- function(theta, data)
  -sum(dpois(data, lambda = theta[1], log = T))
dpois
opt <- nlminb(c(5,5), likelihood_gamma, lower = c(0,0), data = data)

x <- seq(1,400, 1)
plot(x,dgamma(x, opt$par[1], opt$par[2]), 'l')
par(new = T)
hist(data)
```
...
nll <- function(theta, data){
  l1 <- data %>% filter(event == 1) %>% pull(time) %>% dexp(rate = theta) %>% log() %>% sum()
  l2 <- data %>% filter(event == 0) %>% pull(time) %>% 1-pexp(rate = theta) %>% log() %>% sum()
 ' -sum(log(dexp(data[data$event == 1]$time, rate = theta))) - sum(log(pexp(data[data$event == 0]$time, rate = theta)))' 
 -l1-l2
}
act_data %>% filter(event == 1) %>% pull(time) %>% dexp(rate = theta, log = T)
theta <- seq(0, 1e-3, 1e-5)

plot(theta, nll(theta, act_data))
```
