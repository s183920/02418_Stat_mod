...
plot(loc, exp(normalised_LL), 'l',  main = "Normalised Log likelihodd", xlab = "Location parameter", ylab = "Likelihodd")
```
...
for (i in 1:length(loc)){
  ...
  p[i] = profile_mu_cauchy(loc[i], y)
}
normalised_LL_C <- p-opt_profile_cauchy$objective 
...
```{r}
t_dist <- function(theta, data){
    df <- theta[1]
    mu <- theta[2]
    s  <- theta[3]
    -sum(log(gamma((df+1)/2)/(gamma(df/2) * sqrt(pi * df)*s) * (1 + 1/df*(((data-mu)/s)^2))^(-(df+1)/2)))
}
opt <- nlminb(c(0.1,0,0), t_dist, lower = c(0.01, -Inf,0.00001), data = y)

t_dist(c(opt$par[1],opt$par[2],opt$par[3]), y)
```
